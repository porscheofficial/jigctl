...
| R-105 | `state: warn` downgrades a binding's severity to advisory at runtime (D7) | enforced (M1) | requires-record-resolution | the valid `warn` + `severity: blocking` fixture |
| R-106 | severity/cadence defaults are injected per kind tier (D8) | enforced (M1) | requires-record-resolution | the valid fixture that omits both |
| R-107 | an `exceptions[].until` in the past re-fires the constraint | enforced (M1) | requires-record-resolution | the expired-exception tree fixture |
...
**R-101.** `id` uniqueness is checked across every record discovered anywhere in the tree — repo-scoped and every service-scoped directory together — never scoped to a single service's effective set. The algorithm collects the `id` of every record that passed the schema layer, groups records by `id`, and reports one diagnostic per participating record in any group larger than one. Tree-wide scope is deliberate: `supersedes` and `// Implements [[HCR-NNNN]]` source references carry no service qualifier, so two sibling services independently choosing the same id would corrupt both kinds of reference while no single service's effective set would ever contain both colliding records to notice the clash.
...
**R-103.** Every pair of `enforced_by` bindings sharing the same `ref` across the tree must decode to identical `run` strings. Tree-global scope is deliberate: a `ref` carries no service qualifier, so two sibling services could each define `shared-check` with different `run` strings and no single effective set would ever contain both to notice. The comparison happens once both values have been YAML-decoded: a plain `Unmarshal` discards quoting style, so `run: "make lint"` and `run: make lint` decode to the identical Go string and are equal, even though their source quoting differs. When a `ref` group disagrees, the algorithm emits one diagnostic per participating binding, not one for the group as a whole, so every offending binding is individually flagged.
...
**R-105.** A record's effective binding severity is resolved in a fixed order: first preserve an explicitly declared severity or inject the R-106 kind default, then, only when `state: warn`, replace that result with `advisory`. The downgrade therefore defeats even an explicit `severity: blocking`; `draft` and `deprecated` do not participate. Cadence is untouched by the warn downgrade. This rule resolves rather than reports, so the transformation cannot emit a diagnostic and R-105 has no falsifying fixture.

**R-106.** Missing binding values are resolved from the kind tier before R-105 applies. `command`, `config-assert`, `grep`, and `external` default to `severity: blocking` and `cadence: [on-change, ci]`; `agent-review` defaults to `severity: advisory` and `cadence: [scheduled]`; `inferential` defaults to `severity: advisory` and has no cadence default, so its resolved cadence remains empty. Explicit values survive this injection step. After injection, R-105 may downgrade severity for `state: warn`, including an explicit blocking severity, but never changes cadence. This rule resolves rather than reports, so the transformation cannot emit a diagnostic and R-106 has no falsifying fixture.

**R-107.** An exception remains in force through its `until` date and expires only when the current UTC date is strictly after it. The clock is read once per validation invocation and that single date is used for every record. An exception with no `until` is permanent and skipped; an `until` value that matches the schema's shape but is not a calendar date is reported so an unparseable temporary waiver cannot become permanent. Only the stable expired case has a tree fixture, because a fixture asserting non-expiry would itself expire; today and future boundaries are covered by unit tests with a supplied date.
...
**R-110.** Every `external.docs` value must resolve without network I/O. The algorithm treats `docs` as a filesystem path with an optional `#anchor` fragment. If the path contains `://` (e.g. `http://`), the rule skips it entirely — M1 performs no network I/O. This is a deliberately narrow test which does not recognise schemes without an authority component such as `mailto:`. Otherwise, it checks that the path exists relative to the tree root. If an anchor is present, it scans the file for an ATX markdown heading whose jigctl slug matches the anchor exactly. The jigctl slug algorithm (1) strips leading `#`s and surrounding whitespace, (2) lowercases, (3) replaces each run of whitespace with a single `-`, and (4) deletes every character outside `[a-z0-9-]`. Crucially, this is NOT GitHub-style slugging. A file with two `## API` headings has anchors `#api` and `#api-1` under GitHub's rules. The jigctl slug maps both headings to `#api`, so it accepts `#api` but rejects `#api-1`, which GitHub considers valid. Also, markdown emphasis or inline code backticks are not stripped from the heading text.
