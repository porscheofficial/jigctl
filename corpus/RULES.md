...
**R-110.** Every `external.docs` value must resolve without network I/O. The algorithm treats `docs` as a filesystem path with an optional `#anchor` fragment. If the path parses as a URI scheme (e.g. `http://`), the rule skips it entirely — M1 performs no network I/O. Otherwise, it checks that the path exists relative to the tree root. If an anchor is present, it scans the file for an ATX markdown heading whose jigctl slug matches the anchor exactly. The jigctl slug algorithm (1) strips leading `#`s and surrounding whitespace, (2) lowercases, (3) replaces each run of whitespace with a single `-`, and (4) deletes every character outside `[a-z0-9-]`. Crucially, this is NOT GitHub-style slugging. A file with two `## API` headings has anchors `#api` and `#api-1` under GitHub's rules. The jigctl slug maps both headings to `#api`, so it accepts `#api` but rejects `#api-1`, which GitHub considers valid. Also, markdown emphasis or inline code backticks are not stripped from the heading text.
